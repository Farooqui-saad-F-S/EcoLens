...
export default function PollutionCategory({ category }) {
  ...
  return (
    <section aria-labelledby={`${category.id}-category-title`}>
      <div
        className={`relative overflow-hidden rounded-3xl border border-white/10 bg-linear-to-br/srgb ${category.gradient} p-6 sm:p-8`}
      >
        ...
        <div className="relative mt-7 grid gap-2 sm:grid-cols-3">
          {category.flow.map((step, index) => (
            <div
              key={step}
              className="flex items-center gap-2 rounded-xl border border-white/10 bg-canvas/35 px-3 py-2.5 text-xs text-slate-300 backdrop-blur-[4px]"
            >
